...
def register_best_model(top_n: int = 3, delete_unwanted: bool = True) -> None:
    ...
    try:
        ...
        if experiment is None:
            logging.error(f"Experiment '{experiment.experiment_id}' not found.")
            return
        
        experiment_id = experiment.experiment_id

        runs = client.search_runs(
            experiment_id, run_view_type=ViewType.ViewType.ACTIVE_ONLY,
            order_by=["metrics.rmse ASC"],
            filter_string=f"tags.`mlflow.runName` = '{run_name}'"
        )
        
        if not runs:
            logging.warning(f"No runs found in experiment '{experiment_id}'.")
            return
        ...
        if new_model_check:
            registerd_model = mlflow.register_model(model_uri=model_uri, name=os.getenv("REGISTERD_MODEL"))
            registerd_model_version = registerd_model.version
            client.set_registered_model_alias(name=os.getenv("REGISTERD_MODEL"), alias="production", version=registerd_model_version)
...
def compare_models(best_run_id) -> bool:
    try:
        ...
        try:
            ...
            logging.warning(f"Could not load old production model (it may not exist yet). Promoting new model by default.")
            return True
        ...
        if new_model_result < old_model_result:
            ...
            logging.error(f"Faild to register old model as archive even though the new model is performing better.\nold_model result: {old_model_result}\tnew_model result: {new_model_result}")
            return False
        
        logging.warning(f"Old model is performing better than the new trained one.\nold_model_result: {old_model_result}\tnew_model_result: {new_model_result}")
        return False
    
    except Exception as e:
        logging.error("Faild to compare the two models due to {e}")
        raise
...
def archive_old_model() -> bool:
    try:
        ...
        old_model = client.get_model_version_by_alias(name=os.getenv("REGISTERD_MODEL"), alias="production")
        old_model_version = old_model.version

        client.set_registered_model_alias(name=os.getenv("REGISTERD_MODEL"), alias="archive", version=old_model_version)
        return True
    except Exception as e:
        logging.error(f"Cant set alias to the old model due to: {e}")
        raise
